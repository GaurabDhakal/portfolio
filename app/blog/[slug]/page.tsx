...
export default async function SpecificBlog({
  ...
  const highlighter = await getSingletonHighlighter({
    themes: ["tokyo-night"],
    langs: ["typescript", "tsx", "javascript", "jsx", "json", "bash", "css", "html"],
  });

  return (
    <article className="md:max-w-200 overflow-hidden flex flex-col items-center justify-center gap-4">
      <div className="flex flex-col gap-3 px-4">
        <p className="font-extrabold text-2xl md:text-3xl font-pp-neue-montreal antialiased">
          {data.frontmatter.title}
        </p>
        <div className="text-white/70 flex flex-col gap-2">
          <div className="flex items-center gap-1 leading-none text-sm font-mono">
            <p className="m-0 border border-gray-500/50 px-2.5 py-1 bg-[#132221] rounded-full text-[#d7ffdd]">
              <b>Published: </b>
          ...
          <div className="flex gap-2">
            {data.frontmatter.tags.map((tag) => (
              <div key={tag}>
                <BlogTag tagName={tag} className={getTagColor(tag)} />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="px-4">
        <MDXRemote
          ...
          options={{
            mdxOptions: {
              rehypePlugins: [[rehypeShikiFromHighlighter, highlighter, { theme: "tokyo-night" }]],
            },
          }}
        />
      </div>
